...
public class GeologyAndMiningChain implements Chain{
	...
	public JSONArray ServiceSupport(Service request) {
		...
		if (physicsMarks>=6) {
			JO.put("name", "Geology & Mining Eligable");
			nextChain.ServiceSupport(request);
			
		} else {
			System.out.println("Geology & Mining is not allowed");
			JO.put("name", "Geology & Mining Not Eligable! Physics Mark = "+ physicsMarks+" required = 6");
			nextChain.ServiceSupport(request);
		}
